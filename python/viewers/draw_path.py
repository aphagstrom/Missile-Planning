...
def helix_points(path: MsgPath, cycles: int=3, N: int=30):
    '''
        Return points along a helix (for drawing) with:
            center=path.orbit_center
            radius=path.orbit_radius
            direction=path.orbit_direction
            gamma=path.helix_climb_angle
            psi0=helix_start_angle
        N : Each turn in the helix is approximated with an N-point polygon
        cycles: the number of cycles drawn
    '''
    if path.orbit_direction=='CW':
        direction = 1
    if path.orbit_direction=='CCW':
        direction = -1
    theta = path.helix_start_angle
    theta_list = [theta]
    while theta < 2*np.pi:
        theta += direction * 2 * np.pi / N
        theta_list.append(theta)
    points = np.array([[path.orbit_center.item(0) 
                        + path.orbit_radius * np.cos(path.helix_start_angle),
                        path.orbit_center.item(1)
                        + path.orbit_radius * np.sin(path.helix_start_angle),
                        path.orbit_center.item(2)]])
    h_increment = (2 * np.pi * path.orbit_radius) * (2 * np.pi / N) \
        * np.tan(path.helix_climb_angle)
    h = 0
