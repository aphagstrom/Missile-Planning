...
# class DubinsPlanner:
#     def __init__(self, allies_input, enemies_input, targets_input, position, term_yaw, SIM):
...
#         ally_vehicle.path_manager.waypoints.add(ned=ally_vehicle.state.pos, course=ally_vehicle.state.get_euler()[2])
        
#         pseudo_target_pos = ally_vehicle.target.state.pos + sigma * np.array([[1000, 0, 0]]).T + \
#                             16500 * rotz(position) @ np.array([[1000, 0, 0]]).T / np.linalg.norm(np.array([[1000, 0, 0]]))
#         ally_vehicle.path_manager.waypoints.add(ned=pseudo_target_pos, course=term_yaw)
...
#             result = minimize(self.calculate_cost, initial_sigma, 
#                               args=(self.ally_vehicles[j], self.targets[j % len(self.targets)], 
#                                     self.term_yaw[j], self.position[j]),
...
#     sim = DubinsPlanner(allies_input, enemies_input, targets_input, position, term_yaw, SIM)
#     sim.update()
#     sim.simulate()
...
#                      [np.sin(theta), np.cos(theta), 0],
#                      [0, 0, 1]])
# def calculate_cost(sigma, ally_vehicle, target, term_yaw, position):
#     ally_vehicle.path_manager = PathManager(ally_vehicle.dynamics) 
#     ally_vehicle.path_follower = PathFollower(ally_vehicle.dynamics) 
#     ally_vehicle.set_target(target)  
#     ally_vehicle.path_manager.waypoints = MsgWaypoints()
...
#                                             course=ally_vehicle.state.get_euler()[2])
#     pseudo_target_pos = ally_vehicle.target.state.pos + sigma * np.array([[1000,0,0]]).T + 16500 * rotz(position) @ np.array([[1000,0,0]]).T / np.linalg.norm(np.array([[1000,0,0]]))
#     # print("target pos  is:",ally_vehicle.target.state.pos)
#     ally_vehicle.path_manager.waypoints.add(ned=pseudo_target_pos,
#                                             course=term_yaw)
#     ally_vehicle.path_manager.update(ally_vehicle, min_turn_radius)
#     dubins_path_length = ally_vehicle.path_manager.dubins_path.length
...
#         initial_sigma=50
#         for j in range(len(self.ally_vehicles)):
#             result = minimize(calculate_cost, initial_sigma, args=(self.ally_vehicles[j], self.targets[j % len(self.targets)], self.term_yaw[j], self.position[j]),
#                               method='Nelder-Mead', options={'maxiter': 1000})
#             optimal_sigma_j = result.x[0]
#             optimal_sigmas.append(optimal_sigma_j)
#         print("Optimal sigmas are:", optimal_sigmas)
...
# sim.simulate()
    ...
    # def course_has_changed(self, enemy_vehicle, idx, threshold_degrees=np.deg2rad(1)):
    #     enemy_psi = enemy_vehicle.calculate_yaw()  # Assuming calculate_yaw returns psi
        
    #     if enemy_psi is not None:
    #         previous_psi = self.previous_enemy_psi[idx]
    #         if previous_psi is None:
    #             has_changed = True  # Always consider it changed if no previous value
    #         else:
    #             # Check if the absolute difference in yaw angle is greater than threshold_degrees
    #             has_changed = abs(enemy_psi - previous_psi) > threshold_degrees
            
    #         if has_changed:
    #             self.previous_enemy_psi[idx] = enemy_psi  # Update the previous yaw angle
 ...
 #     if self.course_has_changed(self.enemy_vehicles[i], i):
                #         self.ally_vehicles[i].set_target(self.enemy_vehicles[0])  
                # self.run_optimization()
                # self.assign_waypoints()
            # for i in range(len(self.ally_vehicles)):
            #      for j in range(len(self.enemy_vehicles)):
            #         self.check_and_update(i,j) 
    # def check_and_update(self, ally_vehicle_idx,enemy_vehicle_idx):
    #     enemy_vehicle = self.enemy_vehicles[enemy_vehicle_idx]
    #     previous_psi = self.previous_enemy_psi[enemy_vehicle_idx]
    #     enemy_psi = enemy_vehicle.calculate_yaw()
    #     if enemy_psi is not None:
    #         if previous_psi is None or abs(enemy_psi - previous_psi) > np.deg2rad(0.1):
    #             print(f"Course has changed for enemy vehicle {enemy_vehicle.id}. Rerunning optimization and assigning waypoints.")
    #             ally_vehicle = self.ally_vehicles[ally_vehicle_idx]
    #             ally_vehicle.set_target(enemy_vehicle)  
                
    #     self.run_optimization()
    #     self.assign_waypoints()
    #     self.previous_enemy_psi[enemy_vehicle_idx] = enemy_psi


# allies_input, enemies_input, targets_input = generate_input_arrays()
# ally_vehicles, enemy_vehicles, targets = init_agents(allies_input, enemies_input, targets_input)
# def rotz(theta: float):
#     return np.array([[np.cos(theta), -np.sin(theta), 0],
#                     [np.sin(theta), np.cos(theta), 0],
#                     [0, 0, 1]])
...
#     ally_vehicle.path_manager.waypoints.add(ned=ally_vehicle.state.pos,
#                                             course=ally_vehicle.state.get_euler()[2])
#     pseudo_target_pos = ally_vehicle.target.state.pos+sigma*np.array([[500,0,0]]).T+16500*rotz(position) @ np.array([[500,0,0]]).T / np.linalg.norm(np.array([[500,0,0]]))
#     ally_vehicle.path_manager.waypoints.add(ned=pseudo_target_pos,
#                                             course=term_yaw)
#     ally_vehicle.path_manager.update(ally_vehicle, min_turn_radius)
#     dubins_path_length = ally_vehicle.path_manager.dubins_path.length
...
#     cost_j = np.abs(sigma - dubins_time)
#     return cost_j
# sigma_values = np.linspace(0,200,200) 
...
# optimal_sigmas=[]
# initial_sigma=50
# for j in range(len(ally_vehicles)):
#     allies_input, enemies_input, targets_input = generate_input_arrays()
#     ally_vehicles, enemy_vehicles, targets = init_agents(allies_input, enemies_input, targets_input)
#     result = minimize(calculate_cost, initial_sigma, args=(ally_vehicles[j], targets[j % len(targets)],term_yaw[j],position[j]),
#                 method='Nelder-Mead', options={'maxiter': 10000})
#     optimal_sigma_j = result.x[0]
#     optimal_sigmas.append(optimal_sigma_j)
# print("optimal sigmas are:",optimal_sigmas)
# max_sigma=max(optimal_sigmas)
# for i in range(len(ally_vehicles)):
#     ally_vehicles[i].path_manager = PathManager(ally_vehicles[i].dynamics) # TODO: do we want these to live here?
#     ally_vehicles[i].path_follower = PathFollower(ally_vehicles[i].dynamics) # TODO: do we want these to live here?
#     ally_vehicles[i].set_target(targets[i % len(targets)]) # pseudo target assignment
# term_radius=16500
# assign_waypoints(ally_vehicles, targets,term_radius,max_sigma,position,term_yaw)
# sim_time = SIM.start_time
# sync_dubins_paths(ally_vehicles, targets)

# print("dubins path 1 is:",ally_vehicles[0].path_manager.dubins_path.length/ally_vehicles[0].get_airspeed())
# print("dubins path 2 is:",ally_vehicles[1].path_manager.dubins_path.length/ally_vehicles[1].get_airspeed())
# print("dubins path 3 is:",ally_vehicles[2].path_manager.dubins_path.length/ally_vehicles[2].get_airspeed())                
# plot_app = pg.QtWidgets.QApplication([])
# world_view = WorldViewer(
...
#     for i in range(len(ally_vehicles)):
#         if ally_vehicles[i].exists:
#             ally_vehicles[i].update_all()
#     for i in range(len(enemy_vehicles)):
#         if enemy_vehicles[i].exists:
#             enemy_vehicles[i].update_dynamics()
#     if SIM.show_visualizer:  
#         world_view.update(ally_vehicles, enemy_vehicles, targets,term_yaw,position,sim_time)  
#     sim_time += SIM.ts_simulation
# if SIM.plot_end:
#     input('Press RETURN to show plots\n')
# #     plot_data(HSV.num_missiles, True) # TODO
