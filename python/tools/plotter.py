...
mavsim_python
    ...
    - Last Update:
        7/13/2023 - David Christensen
"""
import pyqtgraph as pg
import time

class Plotter():
    def __init__(self, app, plots_per_row, geometry=[0, 0, 1280, 800]):
        self._app = app
        self._window = pg.QtWidgets.QMainWindow()
        self._window.setGeometry(geometry[0], # upper_left_x
                                 geometry[1], # upper_left_y
                                 geometry[2], # width
                                 geometry[3])  # height  
